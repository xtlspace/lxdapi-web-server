...
func InitManager() error {
	...
	var count int64
	db.DB.Model(&models.IPv6Pool{}).Count(&count)
	logger.OK("IPv6管理器初始化成功，地址池: %d个IP", count)
	
	if err := GlobalManager.restoreBindings(); err != nil {
		logger.Warn("恢复IPv6绑定失败: %v", err)
	}
...
func (m *Manager) AllocateIPs(containerName, userID string, count int) ([]string, error) {
	if count == 0 {
		logger.Info("容器 %s 不需要分配IPv6", containerName)
		return nil, nil
	}
	...
	var availablePools []models.IPv6Pool
	...
	if len(availablePools) < count {
		return nil, fmt.Errorf("可用IPv6不足，需要%d个，只有%d个", count, len(availablePools))
	}
	
	var result []string
	for _, pool := range availablePools {
		binding := &models.IPv6Binding{
		...
		if err := db.DB.Create(binding).Error; err != nil {
			logger.Error("保存IPv6绑定记录失败: %v", err)
			continue
		}
		...
		logger.OK("分配IPv6: %s -> %s", pool.IPAddress, containerName)
	}
...
func (m *Manager) BindIP(containerName, ipAddress, containerIP string) error {
	...
	db.DB.Model(&models.IPv6Binding{}).
		Where("container_name = ? AND ip_address = ?", containerName, ipAddress).
		Update("status", "bound")
	
	logger.OK("IPv6绑定成功: %s <-> %s", ipAddress, containerIP)
	return nil
}
...
func (m *Manager) ReleaseIPs(containerName string) error {
	...
	var natConfig models.NATConfigV6
	...
	for _, binding := range bindings {
		...
		if err := db.DB.Where("ip_address = ?", binding.IPAddress).First(&pool).Error; err == nil {
			...
			logger.Warn("未找到IPv6地址池记录: %s，尝试使用默认网卡删除", binding.IPAddress)
			m.removeAllRules(binding.IPAddress)
			if natConfig.Interface != "" {
				m.removeIPFromInterface(binding.IPAddress, natConfig.Interface)
			}
		}
		db.DB.Model(&models.IPv6Pool{}).Where("ip_address = ?", binding.IPAddress).Update("status", "available")
		logger.Info("释放IPv6: %s (容器: %s)", binding.IPAddress, containerName)
	}
	
	db.DB.Unscoped().Where("container_name = ?", containerName).Delete(&models.IPv6Binding{})
	logger.OK("容器 %s 的所有IPv6已释放", containerName)
	return nil
}

func (m *Manager) ReleaseIP(containerName, ipAddress string) error {
	var binding models.IPv6Binding
	if err := db.DB.Where("container_name = ? AND ip_address = ?", containerName, ipAddress).First(&binding).Error; err != nil {
		return fmt.Errorf("IPv6绑定不存在")
	}
	...
	if err := db.DB.Unscoped().Where("container_name = ? AND ip_address = ?", containerName, ipAddress).Delete(&models.IPv6Binding{}).Error; err != nil {
		return fmt.Errorf("删除IPv6绑定记录失败: %v", err)
	}
	
	db.DB.Model(&models.IPv6Pool{}).Where("ip_address = ?", ipAddress).Update("status", "available")
	
	logger.OK("释放IPv6成功: %s (容器: %s)", ipAddress, containerName)
	return nil
}
...
func (m *Manager) restoreBindingsWithRetry(bindings []models.IPv6Binding) {
	for i := 0; i < 30; i++ {
		if m.checkLxdReady() {
			break
		}
	...
	logger.Info("开始恢复 %d 个IPv6绑定...", len(bindings))
	restored := 0
	...
	for _, binding := range bindings {
		...
		if err := db.DB.Where("name = ?", binding.ContainerName).First(&container).Error; err == nil && container.PrivateIPv6 != "" {
			m.addSNAT(binding.IPAddress, container.PrivateIPv6, pool.Interface)
			m.addDNAT(binding.IPAddress, container.PrivateIPv6, pool.Interface)
		}

		restored++
	}
	
	logger.OK("IPv6绑定恢复完成: %d/%d", restored, len(bindings))
...
func (m *Manager) checkLxdReady() bool {
	cmd := exec.Command("nft", "list", "table", "inet", "lxdnat")
	return cmd.Run() == nil
}
